...
type network struct {
	...
	outstandingRequestHandlers map[uint32]message.ResponseHandler // maps basedaigo requestID => message.ResponseHandler
	...
	appSender                  common.AppSender                   // basedaigo AppSender for sending messages
...
// CrossChainAppRequestFailed can be called by the basedaigo -> VM in following cases:
...
// AppRequest is called by basedaigo -> VM when there is an incoming AppRequest from a peer
...
// AppRequestFailed can be called by the basedaigo -> VM in following cases:
// - node is benched
...
// AppGossip is called by basedaigo -> VM when there is an incoming AppGossip from a peer
// error returned by this function is expected to be treated as fatal by the engine
